...
exports.init = (sequelize, optionsArg) => {
			...
			if (
				...
				return revision
					...
					.then(objectRevision => {
						...
						if (options.enableRevisionChangeModel) {
							return Promise.all(_.forEach(delta, difference => {
	...
	return {
		...
		defineModels: function defineModels(db) {
			...
			if (options.mysql) {
				attributes.document.type = sequelize.Sequelize.TEXT('MEDIUMTEXT');
			}
			...
			if (options.enableRevisionChangeModel) {
				...
				if (options.mysql) {
					attributes.document.type = sequelize.Sequelize.TEXT('MEDIUMTEXT');
					attributes.diff.type = sequelize.Sequelize.TEXT('MEDIUMTEXT');
				}
