...
exports.init = (sequelize, optionsArg) => {
	...
	const ns = optsArg.continuationNamespace;
	...
	_.assignIn(sequelize.Sequelize.Model, {
	...
	return {
		...
		defineModels: function defineModels(db) {
			...
			let attributes = {
				...
				operation: sequelize.Sequelize.STRING(7),
			};

			if (options.mysql) {
				attributes.document.type = sequelize.Sequelize.TEXT(
					'MEDIUMTEXT',
				);
			}
			...
			if (options.enableRevisionChangeModel) {
				...
				if (options.mysql) {
					...
					attributes.diff.type = sequelize.Sequelize.TEXT(
						'MEDIUMTEXT',
					);
				}
